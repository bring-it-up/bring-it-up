...
export default function ServiceHoursCard({parentToChild}: {parentToChild: string}): ReactElement {
    ...
    let key: any = weekday[new Date().getDay()];
...
function printHoursForDay(day: string, opens: string, closes: string) {
    const weekday = ["sun","mon","tue","wed","thu","fri","sat"];
    let today: any = weekday[new Date().getDay()];
    let dayString = getDay(day);
    let openingTime = convertTo12HourTime(opens);
    if (today === day) {
...
function convertTo12HourTime(time: string): string {
    ...
    let timeString = new Date('2022-01-01 ' + time).toLocaleString('en-US', { timeZone: 'America/Los_Angeles', hour12: true,hour:'numeric',minute:'numeric'});
    return timeString;
}
